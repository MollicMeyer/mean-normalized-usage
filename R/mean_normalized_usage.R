...
#' normalized usage values by covariate group, analysis scale, depth class, and statistical aggregation
# of terrain and imagery covariates.
#'
#' @param df A data frame like 'cubistusage.csv' with model_conditions, model_usage, covariates, model property, model depth etc.
...
mean_normalized_usage <- function(df) {
  ...
  df <- df %>%
    mutate(
      ...
      analysis.scale = ifelse(is.na(analysis.scale) & Deriv == "Imagery", 3, analysis.scale),
      analysis.scale = ifelse(covariate == "TWI30m", 30, analysis.scale),
      analysis.scale = ifelse(covariate == "HDCN_strahl3_d", 10, analysis.scale),
      rel_scale = case_when(
  ...
  # Order factor levels
  df$rel_scale <- factor(df$rel_scale,
                         levels = c("sub-plot", "plot", "subcatchment", "catchment", "subwatershed",
                                    "watershed", "landscape"),
                         ordered = TRUE
  )
  ...
  df$stat <- factor(df$stat, levels = c("pre-2010", "post-2010", "none", "std", "min", "max", "mean", "median"))

  df$type2 <- factor(df$type2, levels = c(
    "red", "green", "blue", "infrared", "ndvi",
    "slope gradient", "curvature", "local relief",
    "wetness indices", "channel proximity", "aspect"
  ))
